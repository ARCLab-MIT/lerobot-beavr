...
def _find_episode_index(dataset, frame_idx: int) -> int:
    """
    Find the episode index for a given frame index using binary search for efficiency.
    """
    from_indices = dataset.episode_data_index["from"]
    to_indices = dataset.episode_data_index["to"]
    
    # Binary search for the episode containing this frame
    left, right = 0, len(from_indices) - 1
    while left <= right:
        mid = (left + right) // 2
        from_idx = from_indices[mid].item()
        to_idx = to_indices[mid].item()
        
        if from_idx <= frame_idx < to_idx:
            return mid
        elif frame_idx < from_idx:
            right = mid - 1
        else:
            left = mid + 1
    
    # Fallback: return the last episode if frame index is out of bounds
    return len(from_indices) - 1


def _gather_seq_batch(dataset, cur_frame_indices: list[int], l_hist: int) -> dict:
    """
    Build a (B, L_hist, ...) batch around each current frame index.
    Pads from the left when the episode boundary is hit (you can also mark pads).
    Assumes dataset[i] returns a dict of tensors for a single frame.
    """
    B = len(cur_frame_indices)
    # Collect l_hist indices per stream, clamped to episode starts
    seq_indices = []
    for idx in cur_frame_indices:
        # Find which episode this frame belongs to
        episode_idx = _find_episode_index(dataset, idx)
        
        # Build [idx-(L-1), ..., idx], clamping to episode start
        episode_start = dataset.episode_data_index["from"][episode_idx].item()
        start = max(episode_start, idx - (l_hist - 1))
        seq = list(range(start, idx + 1))
        # Left-pad with start to reach l_hist
        if len(seq) < l_hist:
            seq = [start] * (l_hist - len(seq)) + seq
        seq_indices.append(seq)

    # Now fetch and stack
    out: dict[str, torch.Tensor] = {}
    # First item gives keys and shapes
    sample0 = dataset[seq_indices[0][0]]
    for k, v0 in sample0.items():
        if not torch.is_tensor(v0):
            continue
        # Build (B, L_hist, ...) tensor
        stacked = []
        for b in range(B):
            frames = [dataset[i][k] for i in seq_indices[b]]
            stacked.append(torch.stack(frames, dim=0))        # (L_hist, ...)
        out[k] = torch.stack(stacked, dim=0)                  # (B, L_hist, ...)
    
    # Add frame indices for sliding cache optimization
    out["frame_idx_seq"] = torch.tensor(seq_indices, dtype=torch.long)
...
def train(cfg: TrainPipelineConfig):
    ...
    while step < cfg.steps:
        ...
        batch = _gather_seq_batch(dataset, frame_indices, l_hist=cfg.policy.window_size)
        ...
        batch["reset_mask"] = reset_mask.to(device)
