...
class MyInferenceModel(nn.Module):
    ...
    包含:
      ...
      - predict / denormalize
    """
    def __init__(self, checkpoint_path: str, config: MambaConfig, scaler: Scaler):
        """
        :param checkpoint_path: 已训练好的 ckpt 路径
        :param config: 与训练时相同的 MambaConfig
        :param scaler: 已加载的 Scaler 对象
        """
        super().__init__()
        ...
        # Create module alias for checkpoint compatibility
        import sys
        sys.modules['mamba_policy'] = sys.modules['lerobot.policies.dact.mtil_paper.train.mamba_policy']
