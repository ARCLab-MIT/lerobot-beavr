...
class MACTConfig(PreTrainedConfig):
    ...
    # vision_backbone: str = "dinov2_vitb14"
    ...
    def __post_init__(self):
        ...
        valid_backbones = ["resnet", "dinov2"]
        if not any(self.vision_backbone.startswith(prefix) for prefix in valid_backbones):
            raise ValueError(
                f"`vision_backbone` must be one of ResNet or DINOv2 variants. Got {self.vision_backbone}."
            )
    ...
    def action_delta_indices(self) -> list:
        return list[int](range(self.chunk_size))
